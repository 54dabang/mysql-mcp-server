...
@Service
@Slf4j
public class MysqlMcpServerService2 {
    @Autowired
    ...
    public String listTables(@ToolParam(description = "database") String s) {
...
@Tool(description = "Get the schema information of a specific table", name = "get_table_schema")
public String getTableSchema(@ToolParam(description = "database name") String database,
    ...
    try {
        ...
        if (columns.isEmpty()) {
            return JsonUtils.toJsonString(new TextContent(
                "No schema found for table: " + table + " in database: " + database, "text"));
        }

        // 格式化表结构信息
        StringBuilder schemaInfo = new StringBuilder();
        schemaInfo.append(String.format("Table Schema for %s.%s:\n", database, table));
        schemaInfo.append("Column Name | Data Type | Nullable | Default | Comment\n");
        schemaInfo.append("------------|-----------|----------|---------|--------\n");

        for (Map<String, Object> column : columns) {
            schemaInfo.append(String.format("%s | %s | %s | %s | %s\n",
                    column.get("COLUMN_NAME"),
                    column.get("DATA_TYPE"),
                    column.get("IS_NULLABLE"),
                    column.get("COLUMN_DEFAULT") != null ? column.get("COLUMN_DEFAULT") : "NULL",
        ...
        log.error("Failed to get table schema for {}.{}", database, table, e);
        return JsonUtils.toJsonString(new TextContent(
            "Error getting table schema: " + e.getMessage(), "text"));
    }
}
